...
n = int(input("Enter the num of messages:"))
data = {}
for i in range(n):
...
print(data)
while True:
    print("1. Count total num of messages")
    ...
    #6. Find the most active user
        ...
        for name in data:
            if max_count < len(data[name]):
                ...
                user = name
        print(f"Most active user: {user} {max_count} messages")
    ...
    #16. Extract all questions asked in the chat
        ...
        for name in data:
            for idx, msg in data[name]:
                if '?' in msg:
                    list_qus.append(msg)
        print(f"List of messages ending with or containing ?: {list_qus}")
